                ...
                {notifications.map((n, index) => (
                    <div key={index} className="flex items-start space-x-2 sm:space-x-3">
                        <Avatar className="h-8 w-8 flex-shrink-0">
                            <AvatarFallback>{n.sender!.firstname![0].toUpperCase() ?? "N/A"}</AvatarFallback>
                        </Avatar>
                        <div className="flex-1 space-y-1 min-w-0">
                            <p className="text-xs sm:text-sm leading-tight">
                                <span className="font-medium">{n.sender.firstname}</span>{" "}
                                <span className="text-muted-foreground break-words">{n.content}</span>
                            </p>
                        </div>
                        <div className="flex space-x-1 flex-shrink-0">
                            <Button onClick={handleValidNotif} data-sender={n.sender.id} data-entity={n.entityType} data-notif={n.id} size="icon" variant="ghost" className="h-6 w-6">
                                <Check className="h-3 w-3 sm:h-4 sm:w-4 text-white bg-gray-700" />
                                <span className="sr-only">Accept</span>
                            </Button>
                            <Button onClick={handleSupprNotif} data-notif={n.id} size="icon" variant="ghost" className="h-6 w-6">
